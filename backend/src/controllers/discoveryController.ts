...
export const getAuthors = async (req: Request, res: Response) => {
    try {
        ...
        if (keyword) query.$or = [{ name: { $regex: keyword, $options: 'i' } }, { biography: { $regex: keyword, $options: 'i' } }];
        if (language) query.languages = { $in: [String(language)] };
        if (genre) query.genres = { $in: [String(genre)] };
        if (classification) query.classification = String(classification);

        const authors = await Author.find(query).sort({ featured: -1, createdAt: -1 }).limit(20);
        res.json(authors);
    } catch (error: any) {
...
export const getPublishers = async (req: Request, res: Response) => {
    try {
        ...
        const publishers = await Publisher.find(query).sort({ featured: -1, createdAt: -1 }).limit(20);
        res.json(publishers);
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
...
export const getUpcomingBooks = async (req: Request, res: Response) => {
    try {
        ...
        const sortMap: Record<string, any> = {
            releaseDate: { publicationDate: 1 },
            newest: { createdAt: -1 },
            price: { expectedPrice: 1 },
            popularity: { featured: -1, createdAt: -1 }
        };

        const books = await UpcomingBook.find(query).sort(sortMap[String(sort)] || { publicationDate: 1 }).limit(30);
        res.json(books);
    } catch (error: any) {
        res.status(500).json({ message: error.message });
    }
...
export const getAuthorById = async (req: Request, res: Response) => {
    try {
        ...
        if (!author) return;
        const books = await Book.find({ $or: [{ authorId: author._id }, { author: new RegExp(`^${author.name}$`, 'i') }] }).limit(30);
...
export const getPublisherById = async (req: Request, res: Response) => {
    try {
        ...
        if (!publisher) return;
        const books = await Book.find({ $or: [{ publisherId: publisher._id }, { publisher: new RegExp(`^${publisher.name}$`, 'i') }] }).limit(30);
...
export const getBoothById = async (req: Request, res: Response) => {
    try {
        ...
        const books = await Book.find({ $or: [{ _id: { $in: featuredIds.filter((id) => /^[0-9a-fA-F]{24}$/.test(id)) } }, { id: { $in: featuredIds } }] });
...
export const searchDiscovery = async (req: Request, res: Response) => {
    try {
        ...
        const [books, authors, publishers, upcomingBooks, fairs] = await Promise.all([
            ...
            Author.find({ $or: [{ name: regex }, { biography: regex }, { genres: regex }] }).limit(8),
            Publisher.find({ $or: [{ name: regex }, { description: regex }, { genres: regex }] }).limit(8),
            UpcomingBook.find({ $or: [{ title: regex }, { author: regex }, { publisher: regex }, { genre: regex }] }).limit(8),
            BookFair.find({ $or: [{ name: regex }, { city: regex }, { description: regex }] }).limit(8)
        ]);
