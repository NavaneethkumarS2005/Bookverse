    ...
    const fetchRecommendations = async (refresh = false) => {
        ...
        try {
            ...
            const endpoint = token
                ...
                : `/api/recommendations/general${refresh ? '?refresh=true' : ''}`;
        ...
        <>
                ...
                <div className="max-w-7xl mx-auto px-5 grid grid-cols-1 md:grid-cols-2 items-center gap-16 w-full relative z-10">
                    ...
                    <div className="flex justify-center animate-[fadeIn_0.6s_ease-out_0.2s_forwards] opacity-0 relative">
                        <div className="relative z-10">
                            <img src="/images/hero-book.png" alt="Magical Book" className="max-w-full max-h-[550px] drop-shadow-2xl animate-[float_6s_ease-in-out_infinite] transform hover:scale-105 transition-transform duration-500" />
                        </div>
            ...
            <section className="relative overflow-hidden bg-slate-950 py-20 text-white">
                ...
                <div className="relative mx-auto max-w-7xl px-5">
                    ...
                    {recommendationLoading ? <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">{Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-80 animate-pulse rounded-2xl bg-white/10" />)}</div>
                        : recommendations.length ? <div className="grid grid-cols-1 gap-6 sm:grid-cols-2 lg:grid-cols-4">{recommendations.map((book, index) => <ProductCard key={book._id || book.id || `recommendation-${index}`} book={book} />)}</div>
            ...
            <section className="py-24 relative">
                <div className="max-w-7xl mx-auto px-5">
                    ...
                    {loading ? (
                        ...
                        <div className="text-center p-8 rounded-2xl border border-dashed border-red-200 bg-red-50/60 text-red-600 dark:border-red-800 dark:bg-red-900/20 dark:text-red-300">{error}</div>
                    ) : (
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-8">
                            {featuredBooks.map((book, index) => <ProductCard key={book._id || book.id || `featured-${index}`} book={book} />)}
                        </div>
                    )}
                </div>
            </section>
            ...
            <section className="py-24 bg-slate-50 dark:bg-slate-900/50">
                <div className="max-w-7xl mx-auto px-5 grid grid-cols-1 md:grid-cols-2 items-center gap-20">
                    <div>
                        ...
                        <p className="text-slate-500 dark:text-slate-400 text-lg mb-10 leading-relaxed">Dive into our extensive catalog organized by genre and interest. Find exactly what you are looking for.</p>
                        <ul className="grid grid-cols-2 gap-x-8 gap-y-4 mb-10">
                            {['Fiction', 'Sci-Fi', 'History', 'Technology', 'Romance', 'Mystery'].map(genre => (
                                <li key={genre}><Link to={`/categories?genre=${genre}`} className="flex items-center gap-3 text-slate-700 dark:text-slate-300 font-medium hover:text-indigo-600 dark:hover:text-indigo-400 transition-colors group"><span className="text-indigo-500 group-hover:scale-125 transition-transform">•</span> {genre}</Link></li>
                            ))}
                        </ul>
                        <Link to="/categories" className="inline-flex items-center px-6 py-3 rounded-xl font-semibold text-slate-700 dark:text-slate-200 border border-slate-300 dark:border-slate-600 hover:bg-white dark:hover:bg-slate-800 hover:border-indigo-500 transition-all shadow-sm">View All Categories</Link>
                    </div>
                    <div className="flex justify-center items-center"><img src="/images/genre-collection.png" alt="Genre Collection" className="max-w-full h-auto max-h-[400px] drop-shadow-2xl rounded-3xl animate-[float_8s_ease-in-out_infinite_reverse]" /></div>
                </div>
            </section>
            ...
            <section className="py-24 text-center bg-gradient-to-br from-slate-50 to-white dark:from-slate-900 dark:to-slate-950">
                <div className="max-w-4xl mx-auto px-5">
                    ...
                    <p className="text-xl text-slate-500 dark:text-slate-400 mb-10 max-w-lg mx-auto">Sign up today to start your collection or declutter your shelf.</p>
                    <Link to="/login" className="px-10 py-4 rounded-full font-bold text-white bg-gradient-to-r from-pink-500 to-indigo-600 hover:shadow-2xl hover:-translate-y-1 transition-all text-lg shadow-indigo-500/40">Get Started</Link>
                </div>
            </section>
        </>
    );
