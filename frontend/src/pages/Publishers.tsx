...
const FALLBACK_LOGO = 'https://images.unsplash.com/photo-1544716278-ca5e3f4abd8c?auto=format&fit=crop&w=400&h=400&q=80';
...
const Publishers: React.FC = () => {
    ...
    const visiblePublishers = useMemo(() => {
        ...
        return publishers.filter(publisher =>
            [publisher.name, publisher.description, publisher.headquarters, publisher.country, ...(publisher.genres || [])]
                .filter(Boolean)
                .join(' ')
                .toLowerCase()
                .includes(term)
        ...
        <div className="min-h-screen pt-24 pb-12 bg-slate-50 dark:bg-slate-950 transition-colors duration-300">
            <div className="max-w-7xl mx-auto px-5">
                <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-10 gap-4">
                    <div>
                        <h1 className="font-outfit text-4xl font-bold text-slate-900 dark:text-white mb-2">Publishers</h1>
                        <p className="text-slate-500 dark:text-slate-400">Publishing houses, their catalogues and fair presence.</p>
                    </div>
                    <input
                        type="text"
                        value={searchTerm}
                ...
                {loading ? (
                    ...
                    <div className="bg-white dark:bg-slate-900 rounded-3xl p-16 text-center border border-slate-200 dark:border-slate-800 shadow-sm">
                        ...
                        <h3 className="font-outfit text-xl font-bold text-slate-900 dark:text-white mb-2">No publishers found</h3>
                        <p className="text-slate-500 dark:text-slate-400">Publisher profiles will appear here once they are onboarded.</p>
                    </div>
                ) : (
                    <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6">
                        {visiblePublishers.map(publisher => (
                            <DiscoveryCard
                                ...
                                subtitle={publisher.headquarters || publisher.country || 'Location not listed'}
                                description={publisher.description}
                                image={publisher.logo?.url || publisher.logoUrl || FALLBACK_LOGO}
                                badge={publisher.isVerified ? 'Verified' : publisher.genres?.[0] || 'Publisher'}
                                label={publisher.establishedYear ? `Est. ${publisher.establishedYear}` : undefined}
